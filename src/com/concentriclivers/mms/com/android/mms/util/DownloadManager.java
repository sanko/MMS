    ...
    private final BroadcastReceiver mRoamingStateListener =
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_SERVICE_STATE_CHANGED.equals(intent.getAction())) {
                ...
                // TDH: This doesn't appear to actually do anything, so I just
                // commented it out.
...
//                }
    ...
    // TDH: Had to make non-static.
    boolean getAutoDownloadState(SharedPreferences prefs) {
        return getAutoDownloadState(prefs, isRoaming());
    }

    // TDH: Had to make non-static.
    boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming) {
    ...
    // TDH: Had to make non-static for context.
    boolean isRoaming() {
        // TODO: fix and put in Telephony layer
    	
    	// TDH: TODO: Fix properly! We'll assume we're roaming which I think
    	// disables auto-download of mms, but who uses mms anyway?
		// Ok apparently people do use it. I need to use the 
		// hack to get the system property.
        String roaming = SystemPropertiesProxy.get(mContext, 
